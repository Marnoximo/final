...
_DATA_SPLIT = {'plc_test': 'abc/plc_test.tar.gz'}
...
def _get_filenames_with_ids_and_class_names(dataset_dir):
  root = os.path.join(dataset_dir, 'plc_test')
  filenames = []	# files
  media_ids = []
  class_ids = []
  metadata = []	# xml dat
  ...
  id_dict = dict(zip(filenames, class_ids))
  return files, media_dict, id_dict                    # files to class_ids, class_ids to class_names
...
def _convert_dataset(split_name,filenames, media_dict, id_dict, dataset_dir, dataset_name):
        ...
        for shard_id in range(_NUM_SHARDS):
            ...
            with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
                ...
                for i in range(start_ndx, end_ndx):
                    ...
                    class_id = id_dict[filenames[i]]

                    example = dataset_utils.image_to_tfexample_for_test(image_data, b'jpg', height, width, media_id, class_id)
...
def run(dataset_dir, dataset_name):
  ...
  photo_filenames, media_dict, id_dict = _get_filenames_with_ids_and_class_names(dataset_dir)
  ...
  _convert_dataset('test', test_filenames, media_dict, id_dict,
                   dataset_dir, dataset_name)

  dataset_utils.write_dataset_info_file({'test_size': len(test_filenames), 'num_classes': 109, 'dataset_dir': dataset_dir})
